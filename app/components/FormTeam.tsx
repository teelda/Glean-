...
export function FormTeam({ formId, role }: { formId: string; role: string }) {
  ...
  const [details, setDetails] = useState<{ comments: { id: string; body: string; created_at: string }[]; members: { user_id: string; role: string }[]; invitations: { id: string; email: string; role: string; accepted_at: string | null; revoked_at: string | null }[] }>({ comments: [], members: [], invitations: [] });
